...
- `src/components/` owns React rendering, controlled forms, local validation and previews. `src/api/backend.js` is the frontend's only HTTP adapter.
- `src/state.ts` holds the typed API snapshot. A successful write refetches that snapshot instead of patching financial data optimistically.
- `src/components/ui/alert-dialog.tsx` is the only copied Animate UI primitive; it combines Radix focus management with Motion for destructive confirmations.
