...
computed.js      latest(), precioCuota(), cuotasCirc(), calcParticipante(), participantesActivos(), participantesTodos(), historialParticipante(), historialGananciaFondo() — everything derives from S (computed.test.js)
...
utils/           dates.js (incl. todayLocal()), format.js, money-input.js (money-input.test.js), html.js (esc())
